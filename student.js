...
export async function loadTuition(){
  ...
  for(let i=0;i<=currentMonth;i++){
    const status = months[monthNames[i]]?.status || "Unpaid";
    const date = months[monthNames[i]]?.date || "";
    const method = months[monthNames[i]]?.method || "";
...
// Load Break Request (upcoming months)
